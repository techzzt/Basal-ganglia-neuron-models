...
def run_simulation_with_inh_ext_input(neuron_configs, connections, synapse_class, simulation_params, plot_order=None, start_time=0*ms, end_time=10000*ms):
    try:
        ...
        # === Monitors (Only for plot_order or relevant neurons) ===
        spike_monitors = {}
        voltage_monitors = {}
        ...
        for name, group in neuron_groups.items():
            if name in record_neurons:
                ...
                if 'v' in group.variables:
                    v_mon = StateMonitor(group, 'v', record=0, dt=1*ms)  # Only one neuron to reduce memory
        ...
        # === Optional: Spike-Triggered Average (STA) ===
        """
