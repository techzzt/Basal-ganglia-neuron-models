...
class GPi(NeuronModel):
    ...
    def create_neurons(self):
        ...
        if self.connections:
            synapse_eqs = generate_neuron_specific_synapse_inputs("GPi", self.connections)
            full_eqs = base_eqs + '\n' + synapse_eqs
        else:
            full_eqs = base_eqs + '\nIsyn = 0*amp : amp\n'
        
        # Izhikevich 모델에 맞는 threshold와 reset 사용
        self.neurons = NeuronGroup(self.N, full_eqs, threshold='v > vt', reset='v = c; w += d', method='euler')
        ...
        self.neurons.w = self.params['w']['value'] * eval(self.params['w']['unit'])
