...
--- | --- | --- | ---
LIF | $$\frac{dv}{dt} = \frac{-g_L \cdot 1 \, \text{pF/ms/mV} \cdot (v - E_L) + I}{C}$$ | g<sub>L</sub>, E<sub>L</sub>, d, vr, vt, C | 
QIF | $$\frac{dv}{dt} = \frac{k \cdot 1 \, \text{pF/ms/mV} \cdot (v - v_r) \cdot (v - v_t) - u \, \text{pF} + I}{C}$$ <br> <br> $$\frac{du}{dt} = a \cdot \left(b \cdot (v - v_r) - u\right)$$ | a, b, c, C, d, k, v, vr, vt, vpeak, u, I | $$v>vpeak   v = c, u = u + d$$ 
AdEx | $$C_m \frac{dV}{dt} = -g_L(V - E_L) + g_L \Delta_T e^{\frac{V-V_T}{\Delta_T}} - u + I$$ <br> <br> $$\tau_w \frac{du}{dt} = a(V - E_L) - u$$ | g<sub>L</sub>, E<sub>L</sub>, Delta<sub>T</sub>, v, vr, vt, th, tau<sub>w</sub>, a, d, C | v>t<sup>f</sup>  v = c, u = u + d
...
PV+ | GPi |  | | | |
SST+ | GPi |  | | | |
PV+ | SNr | not seperate SNr type [1]   spike cut off refer to [1], and set I arbitarily [2] | | | |
SST+ | SNr |  | | | |
GABAergic | SNc |  | | | |
Dopaminergic | SNc |  | | | |
